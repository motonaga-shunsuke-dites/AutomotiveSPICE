...
const API = (() => {
  ...
  return {
    ...
    listFolders:        (proc)        => req('GET',    `/folders${proc ? '?process=' + proc : ''}`),
    createFolder:       (proc, name)  => req('POST',   '/folders', { process: proc, name }),
    deleteFolder:       (proc, name)  => req('DELETE', '/folders', { process: proc, name }),
    moveDocToFolder:    (docId, folder) => req('PUT',  '/folders/move', { docId, folder }),
